...
export default function HomePage() {
  const { items, loading, ensureThumbnail, getThumbnailUrl } = useStorefrontContext();

  const featuredItems = items.filter((i) => i.IsFeatured).slice(0, 4);
  const displayItems  = featuredItems.length >= 2 ? featuredItems : items.slice(0, 4);
  const displayIds    = new Set(displayItems.map((i) => i.InventoryId));
  const recentItems   = items.filter((i) => !displayIds.has(i.InventoryId)).slice(0, 3);

  useEffect(() => {
    [...displayItems, ...recentItems].forEach((item) => ensureThumbnail(item.InventoryId));
  }, [displayItems, recentItems, ensureThumbnail]);

  // ── Fetch hero slides from the admin-managed endpoint ──
  ...
  return (
    ...
    >
      ...
      {/* 1. Hero — only render when slides are loaded */}
      <div className="relative z-[1]">
        <HeroBanner slides={bannerSlides} interval={6000} />
      </div>

      {/* Trust strip */}
      <div className="relative z-[2] flex items-center justify-center overflow-hidden" style={{ background: "var(--ink)" }}>
        <div className="flex flex-wrap items-center justify-center">
          {["Georgetown Flea Market", "Since 1994", "Washington D.C.", "Antique & Vintage Linens"].map((item, i, arr) => (
            <React.Fragment key={item}>
              <span className="ll-label px-10 py-[1.1rem] text-[0.65rem] uppercase tracking-[0.2em] whitespace-nowrap" style={{ color: "var(--rose-light)" }}>
                {item}
              </span>
              {i < arr.length - 1 && (
                <span className="text-[0.5rem] opacity-60" style={{ color: "var(--rose-deep)" }}>◆</span>
              )}
            </React.Fragment>
          ))}
        </div>
      ...
      {/* 2. Featured pieces */}
      <section className="relative z-[1] px-16 py-24" style={{ background: "var(--cream-dark)" }}>
        <div className="mb-14 flex flex-wrap items-end justify-between gap-4">
          <div>
            <SectionLabel>Handpicked This Week</SectionLabel>
            <SectionTitle>
              Featured <em className="italic" style={{ color: "var(--rose-deep)" }}>Pieces</em>
            </SectionTitle>
          </div>
        </div>

        {loading ? (
          <div className="ll-label py-20 text-center text-[0.75rem] uppercase tracking-[0.2em]" style={{ color: "var(--ink-soft)" }}>
            Loading collection…
          </div>
          ...
          <div className="grid gap-6" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))" }}>
            {displayItems.map((item) => (
              <FeaturedItemCard key={item.InventoryId} item={item} thumbnailUrl={getThumbnailUrl(item.InventoryId)} />
            ))}
          </div>
        )}
      ...
      {/* 3. Shop by category */}
      <section className="relative z-[1] px-16 py-20" style={{ background: "linear-gradient(135deg, var(--sage-light) 0%, var(--cream-dark) 100%)" }}>
        <div className="mb-12 text-center">
          <div className="ll-label mb-3 inline-flex items-center justify-center gap-2 text-[0.62rem] font-medium uppercase tracking-[0.25em]" style={{ color: "var(--sage-deep)" }}>
            What You&apos;ll Find
          </div>
          <SectionTitle center>
        ...
        <div className="grid gap-px" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", background: "var(--linen)" }}>
          {[
            ...
            <Link
              ...
              onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.background = "var(--rose-light)")}
              onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.background = "var(--cream)")}
            >
              <span className="mb-3 block text-3xl">{icon}</span>
              <div className="ll-display text-sm italic" style={{ color: "var(--ink)" }}>{name}</div>
              <div className="ll-label mt-1 text-[0.58rem] uppercase tracking-[0.12em]" style={{ color: "var(--ink-soft)" }}>{sub}</div>
            </Link>
          ))}
        </div>
      </section>
      ...
      {recentItems.length > 0 && (
        <section className="relative z-[1] px-16 py-20" style={{ background: "var(--cream)" }}>
          <div className="mb-12 flex items-end justify-between">
            <div>
              <SectionLabel>Fresh to the Booth</SectionLabel>
              <SectionTitle>
                Just <em className="italic" style={{ color: "var(--rose-deep)" }}>Arrived</em>
              </SectionTitle>
            </div>
            <Link
              href="/shop"
              className="ll-label text-[0.65rem] font-medium uppercase tracking-[0.15em] transition-colors"
              style={{ color: "var(--sage-deep)", textDecoration: "none", borderBottom: "1px solid var(--sage)", paddingBottom: 2 }}
            >
              See all →
            </Link>
          </div>
      ...
      {/* 5. Market schedule */}
      <section id="schedule" className="relative z-[1] grid items-center gap-24 px-16 py-24" style={{ gridTemplateColumns: "1fr 1fr", background: "var(--cream-dark)" }}>
        <div>
        ...
        <div className="relative border p-12" style={{ borderColor: "var(--linen)", background: "var(--cream)" }}>
      ...
      {/* 6. Inquire CTA */}
      <section id="contact" className="relative z-[1] grid items-center gap-12 px-16 py-20" style={{ background: "var(--rose-deep)", gridTemplateColumns: "1fr auto" }}>
        <div>
      ...
      {/* 7. Our story */}
      <section id="about" className="relative z-[1] grid min-h-[60vh]" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <div
        ...
        <div className="flex flex-col justify-center px-16 py-20" style={{ background: "var(--cream)" }}>
